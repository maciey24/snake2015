...
bool UI::sdlInit()
{
    ...
    else
    {
        ...
        #else
        window = SDL_CreateWindow( "Snake", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screenWidth, screenHeight, SDL_WINDOW_SHOWN ); // 2 i 3 argument to pozycja okna - latwiej debugowac jesli otworzysz menedzer zadan obok
        #endif
        if( window == NULL )
        {
...
void UI::sdlClose()
{
    ...
    //SDL_FreeSurface(currentSurface); //nie można zwolnić currentSurface, ponieważ currentSurface wskazuje na któryś z menuSurface, które są zwalniane wcześniej, czyli nullptr
